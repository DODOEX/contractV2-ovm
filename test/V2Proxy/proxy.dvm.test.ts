...
describe("DODOProxyV2.0", () => {
  ...
  before(async () => {
    ...
    dvm_WETH_USDC = await initCreateDVM(ctx, '0x4200000000000000000000000000000000000006', ctx.USDC.options.address, decimalStr("5"), mweiStr("3000"), "5", mweiStr("600"));
  ...
  describe("DODOProxy", () => {
    ...
    it("createDVM - ETH", async () => {
      ...
      var addrs = await ctx.DVMFactory.methods.getDODOPool(ctx.ETH, quoteToken).call();
      assert.equal(
