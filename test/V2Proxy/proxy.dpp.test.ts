...
describe("DODOProxyV2.0", () => {
  ...
  before(async () => {
    ...
    dpp_WETH_USDC = await initCreateDPP(ctx, '0x4200000000000000000000000000000000000006', ctx.USDC.options.address, decimalStr("5"), mweiStr("3000"), "5", mweiStr("600"));
    DPP_WETH_USDC = contracts.getContractWithAddress(contracts.DPP_NAME, dpp_WETH_USDC);
    dpp_WETH_USDT = await initCreateDPP(ctx, '0x4200000000000000000000000000000000000006', ctx.USDT.options.address, decimalStr("5"), mweiStr("3000"), "5", mweiStr("600"));
  ...
  describe("DODOProxy", () => {
    ...
    it("createDPP - ETH", async () => {
      var baseToken = '0x4200000000000000000000000000000000000006';
      var quoteToken = ctx.USDT.options.address;
      ...
      var addrs = await ctx.DPPFactory.methods.getDODOPool("0x4200000000000000000000000000000000000006",quoteToken).call();
      assert.equal(
        await ctx.Web3.eth.getBalance(addrs[1]),
        baseAmount
      );
      assert.equal(
        await ctx.USDT.methods.balanceOf(addrs[1]).call(),
