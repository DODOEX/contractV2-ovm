...
describe("DODOProxyV2.0", () => {
    ...
    before(async () => {
        ...
        cp_DODO_WETH = await initCreateCP(ctx, ctx.DODO.options.address, ctx.ETH, decimalStr("100000"), timeLine, valueList);
    ...
    describe("DODOProxy", () => {
        ...
        it("bid - ETH", async () => {
            ...
            var b_quote = await ctx.Web3.eth.getBalance(cp_DODO_WETH);
            ...
            var a_quote = await ctx.Web3.eth.getBalance(cp_DODO_WETH);
