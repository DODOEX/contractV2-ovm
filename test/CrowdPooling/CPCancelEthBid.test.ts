...
describe("Funding", () => {
  ...
  describe("eth bid & cancel", () => {
    ...
    it("cancel by callee contract", async () => {
      ...
      let receipt = await logGas(ctx.CP.methods.cancel(bidder1, decimalStr("0.02"),"0x00"), ctx.sendParam(bidder1), "cancel and get 0.02 eth")
    ...
    it("claim by callee contract", async () => {
      ...
      let receipt2 = await logGas(await ctx.CP.methods.bidderClaim(bidder2,"0x00"),ctx.sendParam(bidder2),"claim");
      assert.equal(await ctx.QUOTE.methods.balanceOf(bidder2).call(), decimalStr("0.5"))
